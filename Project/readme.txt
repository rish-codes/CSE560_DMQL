...
2. On PGAdmin, open the PSQL tool in the same way we opened the query tool. This should open a terminal. Now open "load.sql" using a notepad app. Make sure to change the absolute path to the location of the files on your system.
 Copy the whole script and paste it on the PSQL terminal and hit Enter. This will load all the data.
 NOTE: If you are facing errors, you might need to run each line individually.
...
4. To run the webapp -
    ...
    4.3. Change parameters in "connect_to_db()" method to reflect the setup of your server.
    4.4. To run the webapp, type "streamlit run website.py" and hit enter. A new browser window should pop up with the webapp running on localhost.
